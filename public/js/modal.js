...
document.addEventListener("click", function (e) {
    ...
    // Evitar abrir modal al hacer clic en marcadores o estrellas
    if (e.target.closest("button, a, .bookmark-icon, .rating-stars-interactive")) return;
    ...
    if (card.classList.contains("tarjeta")) {
        ...
        if (comentariosSection) {
            ...
            if (!usuarioLogueado) {
                const textarea = modalComentarios.querySelector("textarea");
...
function addComment(blogId = null) {
    // 1. Encontrar la sección correcta donde está el botón que disparó la acción
    // Usamos el elemento activo (el botón que se clickeó)
    const button = document.activeElement;

    // 2. Buscar contenedor de comentarios desde ese botón
